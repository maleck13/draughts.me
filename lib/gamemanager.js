  ...
  var self = {
    "createGame":function (opts, cb) {
      ...
      game.on(game.events.GAME_START, function (gamedata){
        console.log("started",gamedata["id"]);
        games.activeGameCount++;
        console.log("Adding to games now there are two players", games);
        games.overallGameCount++;
      });
      game.on(game.events.GAME_READY, function (gameid){
      ...
      game.on('waiting', function (data){
          if(data && data.code === 100){
            if(availableGames.hasOwnProperty(data.id) !== true)availableGames[data.id] = game;
          }
      });
      game.setUp();
    ...
    getGame : function (gameid , cb){
      console.log(availableGames, games);
